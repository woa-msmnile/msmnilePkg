...
~/mu-msmnile/Platforms/SurfaceDuo1Pkg/Device$ tree -L 1 nubia-tp1803/
     ...
     - **Defines.dsc.inc**
       ...
       * *Чтобы получить подробную информацию о макросах, прочтите (DefinesGuidance.md)*
     - **PcdsFixedAtBuild.dsc.inc**
       * *Включено в SurfaceDuo1.dsc.*
       * *Сохраняет персональные данные для определенных устройств. (например, разрешение экрана)*
___
## **Часть 1.** Раннее портирование и тесты.
   - например, портирование uefi для meizu 16T.
     ...
     5. Измените ${brand-codename}/DXE.inc`, `${brand-codename}/APRIORI.inc`, `${brand-codename}/DXE.dsc.inc`.
         + *Ищите разницу в `diff`*
           ...
           < }
             ...
           ```
         + *Поэтому вам нужно отредактировать `Raw Files` в `${brand-codename}/DXE.inc` и добавьте в SimpleTextInOutSerial в `${brand-codename}/DXE.inc` и `${brand-codename}/DXE .dsc.inc`*
     ...
     8. Исправьтеб пропатчите dxe устройства и поместите их `PatchedBinaries/`.
     ...
     11. Соберите uefi.
     12. Проверьте собраное ядро .
         + *Подключите телефон к компьютеру и выполните его на компьютере.*
           ```
           adb reboot bootloader
           fastboot boot Build/meizu-m928q/meizu-m928q.img
           ```
   - В случае успешного портирования ваше устройство войдет в оболочку UEFI.
   - Что делать, если он завис, перезагружается или выходит из строя?
     * Просмотрите часть 3 и исправьте бинарные файлы своего устройства или свяжитесь с нами.
...
## **Часть 2.** Попробуйте загрузить windows.
   ...
   *В случае успешного портирования он загрузится в PE.*
___
## **Часть 3.** Исправление бинарных файлов.
   - Какой бинарный файл требует патча?
     ...
     * Самый простой способ узнать, где патчить:
       ...
       + Dump hex и найдите различия, тогда будет ясно где и что патчить.
         ```
         ...
         - Пример:
             * UFSDxe.efi (nabu):
             ...
             ```
     * Как исправить?
       + Теперь вы знаете, разницу между адресами `0x000025f0` и `0x00004710`.
       ...
       + Откройте xxxDxe.efi своего устройства в IDA (или других програмах).
...
___
## **Приложения**
   – Как получить dtb моего устройства? *допустим в програме termux*
     ...
     * Разделите dtb из boot.img.
       ...
       ```
     * * Переименовать `kernel_dtb`(или `dtb`) на android-`codename`.dtb и поместите его в Device/*\<brand-codename\>*/DeviceTreeBlob/Android/.
     ...
     * Если вы можете загрузить Windows, установите значение `FALSE` и попробуйте.
